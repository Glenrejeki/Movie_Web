...
function createTaskElement(task){
    ...
    deleteBtn.onclick = () => {
        li.remove();
            deleteTaskFromLocalStorage(task); // FIXED: nama fungsi benar
    }
...
function saveTask(){
    ...
    if(editedTask){
        ...
        updatedTaskInLocalStorage(originalTask, editedTask); 
        closeModal()
    }
}
...
function deleteTaskToLocalStorage(task){
    let tasks = JSON.parse(localStorage.getItem('tasks')) || []
...
function updatedTaskInLocalStorage(originalTask, editedTask){
    let tasks = JSON.parse(localStorage.getItem('tasks')) || []; 
